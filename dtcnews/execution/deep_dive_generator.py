...
from execution.topic_tracker import record_topic
...
def main():
    ...
    parser.add_argument(
        "--issue",
        type=int,
        help="Issue number to record this topic for (enables topic tracking)",
    )
    ...
    # Record topic if issue number provided
    if args.issue:
        topic_text = dive.get("headline", "") or (
            content.get("title", "") if isinstance(content, dict) else args.topic
        )
        record_topic(
            topic=topic_text,
            issue_number=args.issue,
            headline=dive.get("headline"),
            source_content=content.get("title") if isinstance(content, dict) else None,
        )
        print(f"\nRecorded topic for issue #{args.issue}: {topic_text[:50]}...")
