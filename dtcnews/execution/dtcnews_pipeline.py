...
DOE_VERSION = "2026.02.04"  # Integrated deep dive into newsletter generator
...
PIPELINE_STEPS = [
    ...
    },
    {
        ...
        "deep_dive_input": "output/deep_dive_{date}.json",
    ...
    try:
        ...
        if step.get("input") and step_id not in ["aggregate"]:
            ...
            if step_id == "filter":
                ...
                # newsletter_generator needs content file, issue number, and deep dive
                cmd.extend(["--content-file", str(input_path)])
                cmd.extend(["--issue-number", "1"])  # TODO: Track issue numbers
                # Add deep dive if available
                if step.get("deep_dive_input"):
                    deep_dive_path = resolve_path(step["deep_dive_input"], date_str)
                    if deep_dive_path.exists():
                        cmd.extend(["--deep-dive-file", str(deep_dive_path)])
