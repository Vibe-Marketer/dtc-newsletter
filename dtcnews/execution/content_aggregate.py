...
1. Fetches viral content via Perplexity search (PRIMARY - no API keys needed)
2. Optionally fetches from Reddit (if API configured)
3. Optionally fetches videos from YouTube
4. Applies deduplication across all sources
5. Generates content sheet with virality analysis (CSV + JSON)
6. Saves results to output/ directory

The search-based approach finds content from Reddit, Twitter, YouTube etc.
without needing individual API access.
"""
...
try:
    from execution import search_fetcher as _search_fetcher
...
Examples:
  ...
  python execution/content_aggregate.py --output-format json
    ...
    parser.add_argument(
        ...
        type=str,
        default="search",
        help="Comma-separated sources: search,reddit,youtube,perplexity (default: search)",
    )
...
def run_aggregation(
    ...
    # Default sources
    if sources is None:
        sources = ["search"]  # Search is primary, doesn't need Reddit API
    ...
    if "reddit" in sources:
        if REDDIT_AVAILABLE:
            ...
            print("  Reddit: API not configured, will skip")
    ...
    # === SEARCH (PRIMARY - uses Perplexity to find viral content) ===
    if "search" in sources:
        print("\n[SEARCH] Fetching viral content via Perplexity search...")
        if SEARCH_AVAILABLE and _search_fetcher is not None:
            try:
                items = _search_fetcher.fetch_all_content(verbose=True)
                # Filter by min score
                items = [i for i in items if i.get("outlier_score", 0) >= min_score]
                all_content.extend(items)
                source_counts["search"] = len(items)
                print(f"  Found {len(items)} items meeting {min_score}x threshold")
            except Exception as e:
                print(f"  Search error: {e}")
                source_counts["search"] = 0
        else:
            print("  Search module not available or PERPLEXITY_API_KEY not set")
            source_counts["search"] = 0

    # === REDDIT ===
    if "reddit" in sources and REDDIT_AVAILABLE:
